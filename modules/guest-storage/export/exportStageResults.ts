...
function slotLabel(
  ...
  if (
    ...
    return (
      entriesById.get(value.entryId)?.display_name ??
      "Unknown entry"
    )
  }

  if (
    value.type === "rotation_team" &&
    ...
    return value.entryIds
      ...
      .map(
        (id) =>
          entriesById.get(id)?.display_name ??
          "Unknown entry",
      )
      .join(" + ")
  }
...
function buildIndividualRotation(
  input: ExportInput,
) {
...
function buildRoundRobin(
  ...
  const standings =
    buildRoundRobinStandings(views)

  const lines = [
    ...commonHeader(input, "Round Robin"),
    row(["STANDINGS"]),
  ]

  for (const group of standings) {
    lines.push(
      row([`Group ${group.groupKey}`]),
    )
    lines.push(
      row([
        "Position",
...
function buildElimination(input: ExportInput) {
  const lines = [
    ...commonHeader(input, "Elimination"),
    ...matchRows(input, "Elimination"),
  ]
...
async function shareCsvOnNative(
  csv: string,
  fileName: string,
) {
  const data = utf8ToBase64(
    `\uFEFF${csv}`,
  )
  ...
  await Share.share({
    title: fileName,
    text: "Pickleball Arena tournament results",
    url: result.uri,
    dialogTitle: "Export CSV",
  })
}

export async function exportStageResultsCsv(
  input: ExportInput,
) {
  const csv =
