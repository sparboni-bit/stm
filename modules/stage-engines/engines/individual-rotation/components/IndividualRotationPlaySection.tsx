...
import { StageRoundTimer } from "../../../../stage-timer/components"
...
export function IndividualRotationPlaySection() {
  ...
  const canAddRound =
    stage.status === "generated" || stage.status === "running"
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="border border-slate-200 bg-slate-50 p-4 sm:p-5">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button
            ...
            className="inline-flex min-h-11 w-full shrink-0 items-center justify-center bg-slate-950 px-6 py-3 text-sm font-bold text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:bg-slate-300 sm:w-auto"
          >
        ...
        {message ? (
          <div role="status" className="mt-4 border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-800">
            {message}
          </div>
        ) : null}

        {error ? (
          <div role="alert" className="mt-4 border border-red-200 bg-red-50 px-4 py-3 text-sm font-medium text-red-700">
            {error}
          </div>
        ) : null}
      </section>
